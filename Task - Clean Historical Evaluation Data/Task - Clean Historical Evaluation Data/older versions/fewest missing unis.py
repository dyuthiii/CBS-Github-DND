...
def check_names_in_map(row):
    ...
    names = [row['name_1'].lower().strip(), row['name_2'].lower().strip(), row['name_3'].lower().strip()]
    names = [n for n in names if n]
    pairs = [(a, b) for a in names for b in names]

    for f, l in pairs:
        candidates = name_ref_df[((name_ref_df['first_name'] == f) & (name_ref_df['last_name'] == l)) |
                                  ((name_ref_df['first_name'] == l) & (name_ref_df['last_name'] == f)) |
                                  (name_ref_df['first_name'] == f) |
                                  (name_ref_df['last_name'] == l)]
